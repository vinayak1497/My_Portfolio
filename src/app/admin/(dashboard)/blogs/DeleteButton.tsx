...
export function DeleteButton({ slug }: { slug: string }) {
  const deleteWithSlug = deleteBlog.bind(null, slug)
  const [, action, pending] = useActionState(deleteWithSlug, undefined)
  return (
    <form action={action} className="inline">
      <button type="submit" disabled={pending} onClick={(e) => { if (!confirm('Delete this blog post? This cannot be undone.')) e.preventDefault() }}
