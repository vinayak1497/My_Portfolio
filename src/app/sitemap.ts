...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const [projects, blogs, notes, certifications, hackathons, certificates] =
    await Promise.all([
      getProjects().catch(() => []),
      getBlogs().catch(() => []),
      getNotes().catch(() => []),
  ...
  const certPages: MetadataRoute.Sitemap = certifications.map((c) => ({
    url: `${SITE_URL}/badges`,
  ...
  const hackathonPages: MetadataRoute.Sitemap = hackathons.map(() => ({
    url: `${SITE_URL}/badges`,
    lastModified: lastMod,
    changeFrequency: 'monthly' as const,
    priority: 0.5,
